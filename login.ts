...
import SwiftHRAdmin from "./admin/admin";
import { auth } from "./firebase/firebase";

@customElement("swift-hr-login")
export default class SwiftHRAdminLogin extends $LitElement(
  css`
    :host {
      display: block;
      position: relative;
  ...
  render() {
    return html` <section ${fullHeight()}>
      <schmancy-grid
        gap="md"
        rows="0.5fr auto auto 1fr"
        justify="stretch"
        align="stretch"
        class="w-full px-6 h-full"
      >
        <span></span>
        <schmancy-typography align="center" type="display" class="text-center">
          <schmancy-animated-text> Jose Personal </schmancy-animated-text>
        </schmancy-typography>
...
declare global {
  interface HTMLElementTagNameMap {
    "swift-hr-login": SwiftHRAdminLogin;
  }
}
